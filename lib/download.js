...
async function dlAction () {
    ...
    return new Promise((resolve, reject) => {
      try {
        const cloneOptions = {};
        if (jsonConfig.checkout) {
          cloneOptions.checkout = jsonConfig.checkout;
        }
        gitClone(jsonConfig.mirror, path.resolve(__dirname, '../template/react-ts'), cloneOptions, function() {
          // 下载成功
